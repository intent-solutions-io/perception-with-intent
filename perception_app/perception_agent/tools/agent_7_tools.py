...
def upsert_author(
    ...
    Args:
        ...
        articles: List of articles from this feed
        feed_metadata: Optional feed-level metadata (title, link, description)

    Returns:
        Result with:
        - author_id (str): Document ID
    ...
    try:
        ...
        # Extract author name (prefer feed metadata, fallback to article author)
        author_name = None
        if feed_metadata:
            author_name = feed_metadata.get('title') or feed_metadata.get('author')
        if not author_name:
            # Try to get from first article with author
            for article in articles:
        ...
        # Extract website URL
        website_url = None
        if feed_metadata and feed_metadata.get('link'):
        ...
        if feed_metadata:
            if feed_metadata.get('description'):
                update_data['feedDescription'] = feed_metadata['description']
        ...
        if doc.exists:
            # Update existing - increment article count
            existing_data = doc.to_dict()
            existing_count = existing_data.get('articleCount', 0)
            update_data['articleCount'] = existing_count + len(articles)
            status = "updated"
        else:
            # New author
            update_data['createdAt'] = now
            update_data['articleCount'] = len(articles)
            status = "created"
